...
# 💡 [기능 1 이식] 읽어줄 언어 복수 선택 및 교차 재생 순서/대기시간 UI
...
# 💡 [기능 2 이식] 크메르어 문장 내 마스터 패턴 하이라이트 적용 엔진
            ...
            if f" {pat} " in padded_clean:
                ...
                for match in re.finditer(regex_str, text_str, re.IGNORECASE):
                    ...
                    if pat_type == "시작" and pat_len == 2:
                        ...
                        if re.search(r'[A-Za-z0-9\u1780-\u17FF]', phrase_prefix): # 크메르어 유니코드 범위 포함
                            continue 
...
if processed_df is not None:
    ...
    if st.session_state.is_continuous_playing or (0 <= target_idx < len(filtered_df)):
        if target_idx < len(filtered_df):
            ...
            # 💡 [기능 3 이식] 설정한 '메인 언어(파란색 글씨)'에만 인덱스 번호 부착 및 위치 스위칭
            if read_langs and read_langs[0] == "한국어":
                top_html = f"<span class='khmer-custom-font' style='color: #0f5132;'>{selected_word_display}</span>"
                ...
                bottom_html = f"<span class='khmer-custom-font' style='color: #3b82f6;'>{num_str}{selected_word_display}</span>"
